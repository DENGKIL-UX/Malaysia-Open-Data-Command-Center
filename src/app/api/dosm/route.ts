...
// Updated: Live API testing confirmed the CORRECT IDs are the long-form ones.
const WRONG_ID_SUGGESTIONS: Record<string, string> = {
  gdp_annual: 'Use "gdp_annual_nominal_supply" instead',
  gdp_qtr: 'Use "gdp_qtr_nominal" instead',
  gdp_state: 'Use "gdp_state_real_supply" instead',
  gdp_annual_real_supply_granular: 'Use "gdp_state_real_supply" instead',
  labour_unemployment: 'Use "lfs_month" instead',
  labour_unemployment_state: 'Use "lfs_state_sex" instead',
  lfs_state: 'Use "lfs_state_sex" instead',
  exchangerates_monthly: 'Use "exchangerates" (WIDE format) instead',
  exchangerates_daily: 'Use "exchangerates_daily_0900" (WIDE format) instead',
  crime_index: 'Use "crime_district" instead',
  population_age: 'Use "population_malaysia" instead',
  births_annual: 'Use "births" instead',
  poverty_absolute: 'Use "hh_poverty_state" instead',
  trade_monthly: 'Use "trade_headline" instead',
  bop: 'Use "bop_balance" instead',
  fdi: 'Use "fdi_flows" instead',
  hies_malaysia: 'Use "hh_income" instead',
  federal_finance_year_revenue: 'Use "federal_finance_year" with variable filter instead',
  // No API at all — must use CSV proxy
  hpi_malaysia: 'No API endpoint exists — use CSV proxy only',
  cpi_2d: 'No separate API — use cpi_headline with division filter',
  cpi_3d: 'No separate API — use cpi_headline with division filter',
  divorces: 'No API endpoint exists — use CSV proxy only',
  pricecatcher_week: 'No API endpoint exists — use CSV proxy only',
  hospital_beds: 'No API endpoint exists — use CSV proxy only',
  healthcare_staff: 'No API endpoint exists — use CSV proxy only',
  electricity_supply: 'No API endpoint exists — use CSV proxy only',
  water_consumption: 'No API endpoint exists — use CSV proxy only',
  vehicle_registration: 'No API endpoint exists — use CSV proxy only',
  road_accidents: 'No API endpoint exists — use CSV proxy only',
  deaths_cause: 'No API endpoint exists — use CSV proxy only',
  lfs_edu: 'No API endpoint exists — use CSV proxy only',
  trade_country: 'No API endpoint exists — use CSV proxy only',
  palm_oil: 'No API endpoint exists — use CSV proxy only',
  population_parlimen: 'No API endpoint exists — use CSV proxy only',
};
